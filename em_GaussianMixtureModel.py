...
class GMM:
    def __init__(self, k, max_steps=2000, independent_variance=True, verbose=True):
        self.k = k
    ...
    def fit(self, X):
        ...
        if self.independent_variance:
            self.std = np.repeat(np.std(X, axis=0, keepdims=True), self.k)
    ...
    def pairwise_likelihood(self, X):
        ...
        if self.independent_variance:
            ...
            data_log_likelihood = -.5 * (dis @ np.linalg.inv(fixed_cov) * dis).sum(axis=-1) \
                                  - .5 * np.linalg.slogdet(2 * np.pi * fixed_cov)[1][:, None]
        data_likelihood = np.exp(data_log_likelihood)
        # the posterior of each datum belonging to a distribution, of shape [k, n]
...
# ---------------------- Example 1---------------------------------------------
    X = np.concatenate([np.random.normal([0, 0], [.3, .3], [100, 2]),
